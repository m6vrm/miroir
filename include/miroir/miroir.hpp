...
struct NodeAccessor {
    ...
    static auto at(const Node& node, const Key& key) -> Node;

    // casts node to type T
    template <typename T>
    static auto as(const Node& node) -> T;
    // casts node to type T or returns a fallback value of the same type
    template <typename T>
    static auto as(const Node& node, const T& fallback) -> T;
...
struct Error {
    ...
    // int max_depth - maximum depth of nested errors (0 = infinite depth, 1 =
    // flat, etc.)
...
class Validator {
   public:
    ...
    using TypeValidator = auto (*)(const Node& value) -> bool;

   public:
    explicit Validator(const Node& schema,
                       const std::map<std::string, TypeValidator>& type_validators = {});

    auto validate(const Node& document) const -> std::vector<Error>;
   ...
   private:
    auto make_error(ErrorType type,
                    const Context& ctx,
                    const std::vector<std::vector<Error>>& variant_errors = {}) const -> Error;

    void validate(const Node& document,
                  const Node& schema,
                  const Context& ctx,
                  std::vector<Error>& errors) const;

    void validate_type(const Node& document,
                       const std::string& type,
                       const Context& ctx,
                       std::vector<Error>& errors) const;
    auto validate_type(const Node& document,
                       const std::string& type,
                       const Context& ctx) const -> bool;

    void validate_scalar(const Node& document,
                         const Node& schema,
                         const Context& ctx,
                         std::vector<Error>& errors) const;
    void validate_sequence(const Node& document,
                           const Node& schema,
                           const Context& ctx,
                           std::vector<Error>& errors) const;
    void validate_map(const Node& document,
                      const Node& schema,
                      const Context& ctx,
                      std::vector<Error>& errors) const;
    ...
    auto make_generic_args(const GenericType& keys,
                           const GenericType& values,
...
#ifndef NDEBUG
#include <iostream>
#define MIROIR_ASSERT(condition, message)                                                    \
    do {                                                                                     \
        if (!(condition)) {                                                                  \
...
auto string_is_prefixed(const std::string& string, const std::string& prefix) -> bool {
    return string.compare(0, prefix.size(), prefix) == 0;
}

auto string_trim_after(const std::string& string, char c) -> std::string_view {
    const std::string::size_type position = string.find(c);

    if (position != std::string::npos) {
        return std::string_view{string}.substr(0, position);
    } else {
        return std::string_view{string};
    }
}

auto string_indent(const std::string& string) -> std::string {
...
template <typename Node>
auto dump_variant_errors(const std::vector<std::vector<Error<Node>>>& variant_errors,
    ...
    for (std::size_t i = 0; i < variant_errors.size(); ++i) {
        ...
        for (const Error<Node>& error : variant_errors[i]) {
            result += impl::string_indent(error.description(max_depth));
        }
    }
...
void filter_undefined_node_errors(std::vector<Error<Node>>& errors, std::size_t embed_count) {
    ...
    errors.erase(std::remove_if(errors.begin(), errors.end(),
                                [&errors, embed_count](const Error& remove_error) -> bool {
                                    if (remove_error.type != ErrorType::UndefinedNode) {
                                        return false;
                                    }

                                    const std::size_t count = std::count_if(
                                        errors.cbegin(), errors.cend(),
                                        [&remove_error](const Error& count_error) -> bool {
                                            return count_error.type == remove_error.type &&
                                                   count_error.path == remove_error.path;
                                        });
    ...
    // remove duplicate errors preserving the order and keeping only the last
    // occurrence of error
    std::set<std::pair<ErrorType, std::string>> visited_errors;
    errors.erase(errors.begin(),
                 std::stable_partition(errors.rbegin(), errors.rend(),
                                       [&visited_errors](const Error& error) -> bool {
                                           if (error.type != ErrorType::UndefinedNode) {
                                               return true;
                                           }

                                           if (visited_errors.contains({error.type, error.path})) {
                                               return false;
                                           }

                                           visited_errors.insert({error.type, error.path});
                                           return true;
                                       })
                     .base());
}
...
template <typename Node>
auto node_is_integer(const Node& node) -> bool {
    using NodeAccessor = NodeAccessor<Node>;
    ...
    const std::string value = NodeAccessor::template as<std::string>(node);
...
template <typename Node>
auto node_is_number(const Node& node) -> bool {
    using NodeAccessor = NodeAccessor<Node>;
...
template <typename Node>
auto node_is_boolean(const Node& node) -> bool {
    using NodeAccessor = NodeAccessor<Node>;
    ...
    static constexpr struct {
        std::string_view true_value, false_value;
    } bool_values[] = {
        {"y", "n"},
        {"yes", "no"},
        {"true", "false"},
    ...
    for (const auto& bool_value : bool_values) {
        if (value == bool_value.true_value || value == bool_value.false_value) {
            return true;
        }
    }
...
template <typename Node>
auto node_is_string(const Node& node) -> bool {
    using NodeAccessor = NodeAccessor<Node>;
...
template <typename Node>
auto Error<Node>::description(int max_depth) const -> std::string {
...
template <typename Node>
auto Validator<Node>::Context::with_embed() const -> Context {
    Context ctx = *this;
    ctx.is_embed = true;
    return ctx;
}
...
Validator<Node>::Validator(const Node& schema,
                           const std::map<std::string, TypeValidator>& type_validators)
    : m_settings{schema_settings(schema)},
      m_types{schema_types(schema)},
      m_root{schema_root(schema)},
      m_validators{type_validators} {
...
template <typename Node>
auto Validator<Node>::validate(const Node& document) const -> std::vector<Error> {
    const Context ctx{m_root};
    std::vector<Error> errors;
    validate(document, m_root, ctx, errors);
    return errors;
}
...
template <typename Node>
auto Validator<Node>::schema_root(const Node& schema) -> Node {
...
template <typename Node>
auto Validator<Node>::make_error(ErrorType type,
                                 const Context& ctx,
    ...
    error.type = type;
    error.path = ctx.path;
    error.expected = ctx.expected;
    error.variant_errors = variant_errors;
    return error;
}

template <typename Node>
void Validator<Node>::validate(const Node& document,
                               const Node& schema,
                               const Context& ctx,
                               std::vector<Error>& errors) const {
    if (NodeAccessor::is_scalar(schema)) {
        validate_scalar(document, schema, ctx, errors);
    } else if (NodeAccessor::is_sequence(schema)) {
        validate_sequence(document, schema, ctx, errors);
    } else if (NodeAccessor::is_map(schema)) {
        validate_map(document, schema, ctx, errors);
    } else {
...
template <typename Node>
void Validator<Node>::validate_type(const Node& document,
                                    const std::string& type,
                                    const Context& ctx,
                                    std::vector<Error>& errors) const {
    // generic args
    // note: generic args can only contain names of other types, but not the
    // types themselves, e.g. "generic<string>" is a valid type, but
    // "generic<[string]>" is not, define and use an alias (e.g. "list<string>")
    if (!ctx.where.empty()) {
        ...
        if (concrete_type_it != ctx.where.end()) {
            ...
            validate_type(document, concrete_type, ctx.with_expected(type).with_where({}), errors);
            return;
        }
    }
    ...
    if (type_is_generic(type)) {
        ...
        for (const auto& [schema_type, schema_type_node] : m_types) {
            ...
            if (generic_type.name == generic_schema_type.name) {
                ...
                validate(document, schema_type_node,
                         ctx.with_expected(type).with_where(generic_args), errors);
                return;
            }
        }
    }
    ...
    if (type_it != m_types.end()) {
        ...
        validate(document, schema_type_node, ctx.with_expected(type).with_where({}), errors);
        return;
    }
    ...
    if (validator_it != m_validators.end()) {
        ...
        if (!type_validator(document)) {
            // node has invalid type
            const Error error = make_error(ErrorType::InvalidValueType, ctx.with_expected(type));
...
template <typename Node>
auto Validator<Node>::validate_type(const Node& document,
                                    const std::string& type,
                                    const Context& ctx) const -> bool {
    std::vector<Error> errors;
    validate_type(document, type, ctx.with_expected(type), errors);
    return errors.empty();
}

template <typename Node>
void Validator<Node>::validate_scalar(const Node& document,
                                      const Node& schema,
                                      const Context& ctx,
                                      std::vector<Error>& errors) const {
    const std::string type = NodeAccessor::template as<std::string>(schema);
    validate_type(document, type, ctx, errors);
}

template <typename Node>
void Validator<Node>::validate_sequence(const Node& document,
                                        const Node& schema,
                                        const Context& ctx,
                                        std::vector<Error>& errors) const {
    ...
    if (schema_size == 0) {
        if (!NodeAccessor::is_sequence(document)) {
            // schema node is an empty sequence but document node is not a
            // sequence
            const Error error = make_error(ErrorType::InvalidValueType, ctx);
            errors.push_back(error);
        }
    ...
    if (tag_is_variant(schema_tag)) {
        for (auto it = NodeAccessor::begin(schema); it != NodeAccessor::end(schema); ++it) {
            if (NodeAccessor::equals(document, *it)) {
        ...
        const Error error = make_error(ErrorType::InvalidValue, ctx);
        errors.push_back(error);
    } else if (schema_size == 1) {
        const Node child_schema_node = NodeAccessor::at(schema, 0);

        if (NodeAccessor::is_sequence(document)) {
            for (std::size_t i = 0; i < NodeAccessor::size(document); ++i) {
                const Node child_document_node = NodeAccessor::at(document, i);
                validate(child_document_node, child_schema_node,
                         ctx.appending_path(std::to_string(i)), errors);
            }
        } else {
        ...
        for (auto it = NodeAccessor::begin(schema); it != NodeAccessor::end(schema); ++it) {
            ...
            validate(document, variant_schema, ctx.with_expected(variant_schema), variant_errors);
        ...
        const Error error = make_error(ErrorType::InvalidValueType, ctx, grouped_errors);
        errors.push_back(error);
    }
}

template <typename Node>
void Validator<Node>::validate_map(const Node& document,
                                   const Node& schema,
                                   const Context& ctx,
                                   std::vector<Error>& errors) const {
    const bool is_map = NodeAccessor::is_map(document);
    ...
    std::vector<std::pair<std::string, Node>> key_types;  // key type, schema value node
    ...
    for (auto it = NodeAccessor::begin(schema); it != NodeAccessor::end(schema); ++it) {
        const Node schema_value_node = it->second;
        const std::string schema_value_tag = NodeAccessor::tag(schema_value_node);

        if (tag_is_embed(schema_value_tag)) {
            if (is_map) {
                validate(document, schema_value_node, ctx.with_embed(), errors);
                ++embed_count;
            }
        } else {
            ...
            if (!impl::string_is_prefixed(key, m_settings.key_type_prefix)) {
                const bool node_is_required = tag_is_required(schema_value_tag);
                const std::optional<Node> child_document_node = find_node(document, key);
                const Context child_ctx = ctx.appending_path(key);

                has_required_nodes = has_required_nodes || node_is_required;

                if (child_document_node.has_value()) {
                    validate(child_document_node.value(), schema_value_node, child_ctx, errors);
                    validated_nodes.push_back(child_document_node.value());
                } else if (node_is_required) {
                    // required node not found
                    const Error error = make_error(ErrorType::NodeNotFound, child_ctx);
                    errors.push_back(error);
                }
            } else {
                const std::string key_type = key.substr(m_settings.key_type_prefix.size());
                key_types.emplace_back(key_type, schema_value_node);
            }
        }
    }
    ...
    // validate key types
    for (const auto& [key_type, schema_value_node] : key_types) {
        const std::string schema_value_tag = NodeAccessor::tag(schema_value_node);
        bool key_type_is_valid = !tag_is_required(schema_value_tag);

        for (auto it = NodeAccessor::begin(document); it != NodeAccessor::end(document); ++it) {
            const Node child_document_value_node = it->second;
            if (impl::nodes_contains_node(validated_nodes, child_document_value_node)) {
                continue;
            }

            const Node child_document_key_node = it->first;
            if (!validate_type(child_document_key_node, key_type, ctx)) {
                continue;
            }
            ...
            const std::string child_key =
                NodeAccessor::template as<std::string>(child_document_key_node);
            validate(child_document_value_node, schema_value_node, ctx.appending_path(child_key),
                     errors);
            validated_nodes.push_back(child_document_value_node);
        }
    ...
    for (auto it = NodeAccessor::begin(document); it != NodeAccessor::end(document); ++it) {
        ...
        const Error error = make_error(ErrorType::UndefinedNode, ctx.appending_path(child_key));
        errors.push_back(error);
    }
...
template <typename Node>
auto Validator<Node>::tag_is_embed(const std::string& tag) const -> bool {
...
template <typename Node>
auto Validator<Node>::find_node(const Node& map,
                                const std::string& key) const -> std::optional<Node> {
    ...
    if (m_settings.ignore_attributes) {
        for (auto it = NodeAccessor::begin(map); it != NodeAccessor::end(map); ++it) {
            ...
            const Node value_node = it->second;
            ...
            if (impl::string_trim_after(node_key, m_settings.attribute_separator[0]) == key) {
                return std::optional<Node>{value_node};
            }
        }
    }
...
auto Validator<Node>::type_is_generic(const std::string& type) const -> bool {
    return m_generic_types_cache.contains(type) ||
...
template <typename Node>
auto Validator<Node>::make_generic_args(const GenericType& keys,
                                        const GenericType& values,
                                        const std::map<std::string, std::string>& where) const
    -> std::map<std::string, std::string> {
    MIROIR_ASSERT(!keys.args.empty(), "generic args are empty");
    MIROIR_ASSERT(!values.args.empty(), "generic args are empty");
    MIROIR_ASSERT(keys.args.size() == values.args.size(), "generic args count mismatch");

    std::map<std::string, std::string> generic_args;

    for (std::size_t i = 0; i < keys.args.size(); ++i) {
        const std::string key = keys.args[i];
        const std::string value = values.args[i];

        const auto it = where.find(value);
        if (it == where.end()) {
