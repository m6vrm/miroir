...
template <typename Node> class Validator {
  ...
  private:
    ...
    struct Context {
        std::string path;
        // note: prohibit assignment to a Node, since it may be a reference
        // type, and assignment to an old node may invalidate/replace internal
        // memory of that node (at least the yaml-cpp YAML::Node has this
        // behavior)
        ...
        explicit Context(
            const Context &other,
            const std::variant<std::monostate, std::string, Node> &expected)
            : path{other.path}, expected{expected}, where{other.where},
              is_embed{other.is_embed} {}

        auto appending_path(const std::string &suffix) const -> Context;
        auto with_expected(const std::variant<std::monostate, std::string, Node>
                               &expected) const -> Context;
        auto with_where(const std::map<std::string, std::string> &where) const
            -> Context;
  ...
  private:
    auto make_error(ErrorType type, const Context &ctx,
                    const std::vector<std::vector<Error>> &variant_errors = {})
        const -> Error;

    void validate(const Node &doc, const Node &schema, const Context &ctx,
                  std::vector<Error> &errors) const;

    void validate_type(const Node &doc, const std::string &type,
                       const Context &ctx, std::vector<Error> &errors) const;
    auto validate_type(const Node &doc, const std::string &type,
                       const Context &ctx) const -> bool;

    void validate_scalar(const Node &doc, const Node &schema,
                         const Context &ctx, std::vector<Error> &errors) const;
    void validate_sequence(const Node &doc, const Node &schema,
                           const Context &ctx,
                           std::vector<Error> &errors) const;
...
auto string_is_prefixed(const std::string &str,
...
auto nodes_contains_node(const std::vector<Node> &nodes,
...
template <typename Node>
auto dump_expected(const std::variant<std::monostate, std::string, Node>
                       &expected) -> std::string {
    ...
    for (auto it = NodeAccessor::begin(node); it != NodeAccessor::end(node);
         ++it) {
        // todo: (c++20) use std::format
...
template <typename Node>
auto dump_variant_errors(
    const std::vector<std::vector<Error<Node>>> &variant_errors,
...
template <typename Node>
void filter_undefined_node_errors(std::vector<Error<Node>> &errors,
                                  std::size_t embed_count) {
    ...
    errors.erase(errors.begin(),
                 std::stable_partition(
                     ...
                     [&visited_errors](const Error &err) -> bool {
                         ...
                         if (visited_errors.contains({err.type, err.path})) {
                             return false;
                         }
...
auto Error<Node>::description(int max_depth) const -> std::string {
    ...
    case ErrorType::InvalidValueType:
        return path +
               ": expected value type: " + impl::dump_expected(expected) +
    ...
    case ErrorType::MissingKeyWithType:
        return path +
               ": missing key with type: " + impl::dump_expected(expected);
...
template <typename Node>
auto Validator<Node>::Context::appending_path(const std::string &suffix) const
    -> Context {
...
auto Validator<Node>::Context::with_expected(
    const std::variant<std::monostate, std::string, Node> &expected) const
    -> Context {
...
template <typename Node>
auto Validator<Node>::Context::with_where(
    const std::map<std::string, std::string> &where) const -> Context {
...
auto Validator<Node>::schema_settings(const Node &schema) -> SchemaSettings {
    ...
    if (NodeAccessor::is_defined(settings_node)) {
        settings.default_required = NodeAccessor::as(
            NodeAccessor::at(settings_node, "default_required"),
            settings.default_required);
        settings.optional_tag =
            NodeAccessor::as(NodeAccessor::at(settings_node, "optional_tag"),
                             settings.optional_tag);
        settings.required_tag =
            NodeAccessor::as(NodeAccessor::at(settings_node, "required_tag"),
                             settings.required_tag);
        settings.embed_tag = NodeAccessor::as(
            NodeAccessor::at(settings_node, "embed_tag"), settings.embed_tag);
        settings.variant_tag =
            NodeAccessor::as(NodeAccessor::at(settings_node, "variant_tag"),
                             settings.variant_tag);
        settings.key_type_prefix =
            NodeAccessor::as(NodeAccessor::at(settings_node, "key_type_prefix"),
                             settings.key_type_prefix);
        settings.generic_brackets = NodeAccessor::as(
            NodeAccessor::at(settings_node, "generic_brackets"),
            settings.generic_brackets);
        settings.generic_separator = NodeAccessor::as(
            NodeAccessor::at(settings_node, "generic_separator"),
            settings.generic_separator);
        settings.attribute_separator = NodeAccessor::as(
            NodeAccessor::at(settings_node, "attribute_separator"),
            settings.attribute_separator);
        settings.ignore_attributes = NodeAccessor::as(
            NodeAccessor::at(settings_node, "ignore_attributes"),
            settings.ignore_attributes);
    }
    ...
    MIROIR_ASSERT(!settings.key_type_prefix.empty(),
                  "key type prefix is empty");

    MIROIR_ASSERT(settings.generic_brackets.size() == 2,
                  "invalid generic brackets string length: "
                      << settings.generic_brackets);
...
template <typename Node>
auto Validator<Node>::schema_types(const Node &schema)
    -> std::map<std::string, Node> {

    return NodeAccessor::as(NodeAccessor::at(schema, "types"),
                            std::map<std::string, Node>{});
}
...
auto Validator<Node>::schema_root(const Node &schema) -> Node {
    ...
    MIROIR_ASSERT(NodeAccessor::is_defined(root),
                  "missing root node in the schema");
    return root;
}

template <typename Node>
auto Validator<Node>::make_error(
    ErrorType type, const Context &ctx,
...
template <typename Node>
void Validator<Node>::validate(const Node &doc, const Node &schema,
                               const Context &ctx,
                               std::vector<Error> &errors) const {
    ...
    if (NodeAccessor::is_scalar(schema)) {
        ...
        MIROIR_ASSERT(false,
                      "invalid schema node: " << NodeAccessor::dump(schema));
    }
}

template <typename Node>
void Validator<Node>::validate_type(const Node &doc, const std::string &type,
                                    const Context &ctx,
                                    std::vector<Error> &errors) const {
    ...
    if (!ctx.where.empty()) {
        ...
        if (concrete_type_it != ctx.where.end()) {
            ...
            validate_type(doc, concrete_type,
                          ctx.with_expected(type).with_where({}), errors);
            return;
        }
    }
    ...
    if (type_is_generic(type)) {
        ...
        for (const auto &[schema_type, schema_type_node] : m_types) {
            ...
            const GenericType generic_schema_type =
                parse_generic_type(schema_type);
            if (generic_type.name == generic_schema_type.name) {
                const std::map<std::string, std::string> generic_args =
                    make_generic_args(generic_schema_type, generic_type,
                                      ctx.where);
                validate(doc, schema_type_node,
                         ctx.with_expected(type).with_where(generic_args),
                         errors);
                return;
            }
        }
    ...
    if (type_it != m_types.end()) {
        ...
        validate(doc, schema_type_node, ctx.with_expected(type).with_where({}),
                 errors);
        return;
    }
    ...
    if (validator_it != m_validators.end()) {
        ...
        if (!type_validator(doc)) {
            ...
            const Error err = make_error(ErrorType::InvalidValueType,
                                         ctx.with_expected(type));
...
template <typename Node>
void Validator<Node>::validate_scalar(const Node &doc, const Node &schema,
                                      const Context &ctx,
                                      std::vector<Error> &errors) const {
...
template <typename Node>
void Validator<Node>::validate_sequence(const Node &doc, const Node &schema,
                                        const Context &ctx,
                                        std::vector<Error> &errors) const {
    ...
    if (tag_is_variant(schema_tag)) {
        for (auto it = NodeAccessor::begin(schema);
             it != NodeAccessor::end(schema); ++it) {
        ...
        if (NodeAccessor::is_sequence(doc)) {
            for (std::size_t i = 0; i < NodeAccessor::size(doc); ++i) {
                ...
                validate(child_doc_node, child_schema_node,
        ...
        for (auto it = NodeAccessor::begin(schema);
            ...
            validate(doc, variant_schema, ctx.with_expected(variant_schema),
                     variant_errors);
        ...
        const Error err =
            make_error(ErrorType::InvalidValueType, ctx, grouped_errors);
        errors.push_back(err);
    }
}

template <typename Node>
void Validator<Node>::validate_map(const Node &doc, const Node &schema,
                                   const Context &ctx,
                                   std::vector<Error> &errors) const {
    ...
    std::vector<std::pair<std::string, Node>>
        key_types; // key type, schema val node

    std::size_t embed_count = 0;
    bool has_required_nodes = false;

    // validate document structure
    for (auto it = NodeAccessor::begin(schema); it != NodeAccessor::end(schema);
         ++it) {
        ...
        if (tag_is_embed(schema_val_tag)) {
            ...
            const std::string key =
                NodeAccessor::template as<std::string>(schema_key_node);
            ...
            if (!impl::string_is_prefixed(key, m_settings.key_type_prefix)) {
                ...
                if (child_doc_node.has_value()) {
                    validate(child_doc_node.value(), schema_val_node, child_ctx,
                             errors);
                    validated_nodes.push_back(child_doc_node.value());
                } else if (node_is_required) {
                    // required node not found
                    const Error err =
                        make_error(ErrorType::NodeNotFound, child_ctx);
                    errors.push_back(err);
                }
            } else {
                const std::string key_type =
                    key.substr(m_settings.key_type_prefix.size());
    ...
    for (const auto &[key_type, schema_val_node] : key_types) {
        ...
        for (auto it = NodeAccessor::begin(doc); it != NodeAccessor::end(doc);
             ++it) {
            const Node child_doc_val_node = it->second;
            if (impl::nodes_contains_node(validated_nodes,
                                          child_doc_val_node)) {
                continue;
            }
            ...
            validate(child_doc_val_node, schema_val_node,
                     ctx.appending_path(child_key), errors);
            validated_nodes.push_back(child_doc_val_node);
        }

        if (!key_type_is_valid) {
            // didn't find a key with required type
            const Error err = make_error(ErrorType::MissingKeyWithType,
                                         ctx.with_expected(key_type));
            errors.push_back(err);
        }
    }
    ...
    for (auto it = NodeAccessor::begin(doc); it != NodeAccessor::end(doc);
        ...
        const Error err =
            make_error(ErrorType::UndefinedNode, ctx.appending_path(child_key));
        errors.push_back(err);
    }
...
auto Validator<Node>::find_node(const Node &map, const std::string &key) const
    ...
    if (m_settings.ignore_attributes) {
        for (auto it = NodeAccessor::begin(map); it != NodeAccessor::end(map);
             ++it) {
            const Node key_node = it->first;
            const Node val_node = it->second;
            const std::string node_key =
                NodeAccessor::template as<std::string>(key_node);

            if (impl::string_trim_after(
                    node_key, m_settings.attribute_separator[0]) == key) {
...
template <typename Node>
auto Validator<Node>::parse_generic_type(const std::string &type) const
    -> GenericType {
    ...
    for (auto it = type.cbegin(); it != type.cend(); ++it) {
        MIROIR_ASSERT(state != ST_END,
                      "invalid generic parser intermediate state: " << type);
        ...
        case ST_NAME:
            ...
            break;
        case ST_ARGS:
            arg = std::string_view{!arg.empty() ? arg.cbegin() : &(*it),
                                   arg.size() + 1};
            break;
        case ST_SEP:
            state = ST_ARGS;
    ...
    MIROIR_ASSERT(state == ST_END,
                  "invalid generic parser end state: " << type);
    MIROIR_ASSERT(!generic_type.name.empty(),
                  "generic name is empty: " << type);
    MIROIR_ASSERT(!generic_type.args.empty(),
                  "generic args are empty: " << type);
...
template <typename Node>
auto Validator<Node>::make_generic_args(
    const GenericType &keys, const GenericType &vals,
    ...
    MIROIR_ASSERT(keys.args.size() == vals.args.size(),
                  "generic args count mismatch");
